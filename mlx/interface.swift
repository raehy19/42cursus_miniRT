...
{
	let mlx_ptr:MlxMain = _mlx_bridge(ptr:mlxptr)
	mlx_ptr.inLoop = true
        NSApp.run()
}
...
{
		/// print("Got mlx_ptr ptr \(mlxptr) ")
		let mlx_ptr:MlxMain = _mlx_bridge(ptr:mlxptr)
		let mw = MlxWin(device: mlx_ptr.device, width: Int(w), height: Int(h), title: String(cString: t))

/**		withUnsafePointer(to: &mlx_ptr) {
		    print(" got mlx_ptr init back bridge ptr: \($0)")
	  	 }
**/
		mw.setNotifs()
		mlx_ptr.addWinToList(mw)
...
{
	let win_ptr:MlxWin = _mlx_bridge(ptr:winptr)
	win_ptr.addHook(index: 3, fct: fctptr, param: paramptr)
	return (Int32(0));
}
...
{
        let win_ptr:MlxWin = _mlx_bridge(ptr:winptr)
        win_ptr.addHook(index: 4, fct: fctptr, param: paramptr)
        return (Int32(0));
}
...
{
        let win_ptr:MlxWin = _mlx_bridge(ptr:winptr)
        win_ptr.addHook(index: Int(xevent), fct: fctptr, param: paramptr)
        return (Int32(0));
}
...
{
        let win_ptr:MlxWin = _mlx_bridge(ptr:winptr)
        win_ptr.addHook(index: 12, fct: fctptr, param: paramptr)
        return (Int32(0));
}
...
{
	let mlx_ptr:MlxMain = _mlx_bridge(ptr:mlxptr)
	mlx_ptr.addLoopHook(fctptr, paramptr)
        return (Int32(0));
}
...
{
	let mlx_ptr:MlxMain = _mlx_bridge(ptr:mlxptr)
	mlx_ptr.winList.forEach{ $0.setKeyRepeat(0) }
}
...
{
	let mlx_ptr:MlxMain = _mlx_bridge(ptr:mlxptr)
	mlx_ptr.winList.forEach{ $0.setKeyRepeat(1) }
}
...
{
	let mlx_ptr:MlxMain = _mlx_bridge(ptr:mlxptr)
	let img = MlxImg(d:mlx_ptr.device, w:Int(width), h:Int(height))
	mlx_ptr.addImgToList(img)
...
public func mlx_put_image_to_window_swift(_ mlxptr:UnsafeRawPointer, _ winptr:UnsafeRawPointer, _ imgptr:UnsafeRawPointer, _ x:Int32, _ y:Int32) -> Int32
{
	let win_ptr:MlxWin = _mlx_bridge(ptr:winptr)
	let img:MlxImg = _mlx_bridge(ptr:imgptr)
	win_ptr.putImage(image:img, x:x, y:y)
	return Int32(0)
}
...
public func mlx_put_image_to_window_scale_swift(_ mlxptr:UnsafeRawPointer, _ winptr:UnsafeRawPointer, _ imgptr:UnsafeRawPointer, _ sx:Int32, _ sy:Int32, _ sw:Int32, _ sh:Int32, _ dx:Int32, _ dy:Int32, _ dw:Int32, _ dh:Int32, _ color:UInt32) -> Int32
{
	let win_ptr:MlxWin = _mlx_bridge(ptr:winptr)
	let img:MlxImg = _mlx_bridge(ptr:imgptr)
	win_ptr.putImageScale(image:img, sx:sx, sy:sy, sw:sw, sh:sh, dx:dx, dy:dy, dw:dw, dh:dh, c:color)
	return Int32(0)
}
...
public func mlx_do_sync_swift(_ mlxptr:UnsafeRawPointer) -> Int32
{
	...
	mlx_ptr.winList.forEach { $0.waitForGPU() }
	return Int32(0)
}
...
public func mlx_destroy_window_swift(_ mlxptr:UnsafeRawPointer, _ winptr:UnsafeRawPointer) -> Int32
{
	...
	let win_ptr:MlxWin = _mlx_bridge_transfer(ptr:winptr)
	win_ptr.destroyWinE()
	return Int32(0)
}
...
{
	let win_ptr:MlxWin = _mlx_bridge(ptr:winptr)
	let frame = win_ptr.getWinEFrame()
///	let sframe = win_ptr.getScreenFrame()
	var pt = CGPoint()
